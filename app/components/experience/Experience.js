...
const Experience = () => {
    const { viewport } = useThree();
    const ringScale = viewport.width < 7 ? 1.5 : 4;
    const [isMobile, setisMobile] = useState(viewport.width < 7)
    ...
    return (
        <>
            ...
            <ContactShadows resolution={1024} frames={1} position={[0, -0.5, 0.2]} scale={15} blur={0.5} opacity={1} far={20} />

            {
                isMobile ? <>
                    <mesh scale={ringScale} position={[1, 0, 0]} rotation={[-Math.PI / 2, 0, 0]}>
                        <ringGeometry args={[0.9, 1, 4, 1]} />
                        <meshStandardMaterial color="white" roughness={0.75} />
                    </mesh>
                    <mesh scale={ringScale} position={[-1.5, 0, 0.2]} rotation={[-Math.PI / 2, 0, Math.PI / 2.0]}>
                        <ringGeometry args={[0.9, 1, 3, 1]} />
                        <meshStandardMaterial color="white" roughness={0.75} />
                    </mesh>
                </> : <>
                    <mesh scale={ringScale} position={[3, 0, 0]} rotation={[-Math.PI / 2, 0, 0]}>
                        <ringGeometry args={[0.9, 1, 4, 1]} />
                        <meshStandardMaterial color="white" roughness={0.75} />
                    </mesh>
                    <mesh scale={ringScale} position={[-3, 0, 0.2]} rotation={[-Math.PI / 2, 0, Math.PI / 2.0]}>
                        <ringGeometry args={[0.9, 1, 3, 1]} />
                        <meshStandardMaterial color="white" roughness={0.75} />
                    </mesh></>
            }
